...
def insert_staging_data(cursor, data: Tuple) -> None:
    ...
    cursor.execute("""
        INSERT INTO staging_hubproduct (
            product_id,
            hashkeyproduct,
            dt_load_date,
            recordsource
    ...
    try:
        if connection.is_connected():
            ...
            cursor.execute("""
                ...
                VALUES ('hubproduct', '2023-01-01 00:00:00')
            ...
            cursor.execute("""
                ...
                LEFT JOIN
                    hubproduct h ON s.product_id = h.product_id
                WHERE
            ...
            cursor.execute("""
                UPDATE control_carga
                SET
                    last_load_date = (
                        SELECT MAX(dt_load_date)
                        FROM staging_hubproduct
                    )
                WHERE
                    table_name = 'hubproduct';
            """)
            ...
            cursor.execute("TRUNCATE TABLE staging_hubproduct;")
